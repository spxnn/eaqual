...
/////////////////////////////////////////////////////////////////////////
// local function declarations (see below)
static inline void fft_rstage(  zINT32 n, 
                                zINT32 n2, 
                                zINT32 n4, 
                                zFLOAT *x1, 
                                zFLOAT *x2, 
...
static inline void fft_rbitrev( zFLOAT *x, 
                                zINT32 m);

void    CBasicUtils::zplRealFFT(zFLOAT *x,zINT32 m)
{
 zINT32 n,is,id,i0,n2,n4,k;
 zFLOAT t1;
x--;
 n=1<<m;
 ...
 for (k=2;k<=m;k++)
    {
     n2 = n2*2;
     n4 = n2/4;
     fft_rstage(n,n2,n4,&x[0],&x[n4],&x[2*n4],&x[3*n4]);
    }
 return ;
}
...
C===================================================================C
*/
static inline void fft_rstage(zINT32 n, zINT32 n2, zINT32 n4, zFLOAT *x1, zFLOAT *x2, zFLOAT *x3, zFLOAT *x4)
{
 zINT32 n8,is,id,i1,i2,j,jn;
 zFLOAT t1,t2,e,t3,t4,t5;
 zFLOAT ss1,sd1,ss3,sd3;
 zFLOAT cc1,cd1,cc3,cd3;
 n8 = n2 / 8;
 is = 0;
 id = n2*2;
...
/*C*/
 ...
 id = n2*2;
  do    
    ...
    {
     t1     = (x3[i2] + x4[i2]) * .7071067811865475F;
     t2     = (x3[i2] - x4[i2]) * .7071067811865475F;
     x4[i2] =  x2[i2] - t1;
     x3[i2] = -x2[i2] - t1;
     x2[i2] =  x1[i2] - t2;
     x1[i2] =  x1[i2] + t2;
...
/*C*/
 ...
 cc1 = zplCOS(e);
 cd1 = cc1;
 cd3 = (zFLOAT)(4*(cd1*cd1*cd1) - 3.*cd1);
 cc3 = cd3;
...
CC=================================================================CC
*/
static inline void fft_rbitrev(zFLOAT *x, zINT32 m)
{
 zINT32 itab[257];
 zINT32 m2,nbit,imax,lbss,i,j,k,l,j0;
 zFLOAT t1;
