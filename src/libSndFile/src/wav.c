...
{	WAV_FMT			wav_fmt ;
	...
	{	case 	WAVE_FORMAT_PCM :
		...
		case	WAVE_FORMAT_MS_ADPCM : 
					...
					if ((error = wav_msadpcm_reader_init (psf, &wav_fmt)))
						return error ;
					return 0 ;
...
{	unsigned int	bytesread, k, structsize, bytespersec = 0  ;
	...
	{	case WAVE_FORMAT_PCM :
		...
		case WAVE_FORMAT_MULAW :
				...
				psf->bytewidth      = 1 ;
				if (structsize >= 18)
				{	bytesread += psf_binheader_readf (psf, "w", &(wav_fmt->size20.extrabytes)) ;
		...
		case WAVE_FORMAT_IMA_ADPCM :
				...
				bytesread += 
				psf_binheader_readf (psf, "ww", &(wav_fmt->ima.extrabytes), &(wav_fmt->ima.samplesperblock)) ;
		...
		case WAVE_FORMAT_MS_ADPCM :
				...
				bytesread += 
				psf_binheader_readf (psf, "www", &(wav_fmt->msadpcm.extrabytes), 
				...
				{	bytesread += 
					psf_binheader_readf (psf, "ww", &(wav_fmt->msadpcm.coeffs [k].coeff1), &(wav_fmt->msadpcm.coeffs [k].coeff2)) ;
					snprintf ((char*) psf->buffer, sizeof (psf->buffer), "     %2d     %7d   %7d\n", k, wav_fmt->msadpcm.coeffs [k].coeff1, wav_fmt->msadpcm.coeffs [k].coeff2) ;
		...
		case WAVE_FORMAT_GSM610 :
				...
				bytesread += 
				psf_binheader_readf (psf, "ww", &(wav_fmt->gsm610.extrabytes), &(wav_fmt->gsm610.samplesperblock)) ;
...
{	unsigned int 	fmt_size ;
	...
	{	case	SF_FORMAT_PCM : 
		...
		case	SF_FORMAT_ULAW : 
					psf->sf.format    = (SF_FORMAT_WAV | subformat) ;
					ulaw_write_init (psf) ;

					fmt_size = 2 + 2 + 4 + 4 + 2 + 2 ;
					
					/* fmt : format, channels, samplerate */
					psf_binheader_writef (psf, "lwwl", fmt_size, WAVE_FORMAT_MULAW, psf->sf.channels, psf->sf.samplerate) ;
					/*  fmt : bytespersec */
					psf_binheader_writef (psf, "l", psf->sf.samplerate * psf->bytewidth * psf->sf.channels) ;
					/*  fmt : blockalign, bitwidth */
					psf_binheader_writef (psf, "ww", psf->bytewidth * psf->sf.channels, 8) ;
		...
		case	SF_FORMAT_ALAW : 
					...
					/* fmt : format, channels, samplerate */
					psf_binheader_writef (psf, "lwwl", fmt_size, WAVE_FORMAT_ALAW, psf->sf.channels, psf->sf.samplerate) ;
					/*  fmt : bytespersec */
					psf_binheader_writef (psf, "l", psf->sf.samplerate * psf->bytewidth * psf->sf.channels) ;
					/*  fmt : blockalign, bitwidth */
					psf_binheader_writef (psf, "ww", psf->bytewidth * psf->sf.channels, 8) ;
		...
		case	SF_FORMAT_IMA_ADPCM : 
					...
					if ((error = wav_ima_writer_init (psf)))
						return error ;

					/* File header was fully written in wav_ima_writer_init () so return. */
					return 0 ;
					break ;
		...
		case	SF_FORMAT_MS_ADPCM : 
					...
					if ((error = wav_msadpcm_writer_init (psf)))
						return error ;

					/* File header was fully written in wav_wav_msadpcm_writer_init () so return. */
					return 0 ;
					break ;
		...
		case	SF_FORMAT_GSM610 : 
					...
					if ((error = wav_gsm610_writer_init (psf)))
						return error ;

					/* File header was fully written in wav_gsm610_writer_init () so return. */
					return 0 ;
					break ;
